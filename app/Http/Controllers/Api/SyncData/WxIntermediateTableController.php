...
class WxIntermediateTableController extends Controller
{
    ...
    public function truncatePub(){

        // 公用版处理方式
        $companyList = config('companyList.comanyidList');
        foreach ($companyList as $companyId){
            echo  $companyId.'---ok<br/>';
            //config(['database.connections.mysql.database' => 'work_wechat' . $companyId]);
            DB::table("work_wechat".$companyId.".wx_intermediate_table")->truncate();
        }
    }
