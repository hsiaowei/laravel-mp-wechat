...
@section('content')
    <div id="overtimeList" v-cloak>
        ...
        <div class="details_data_list">
            <div calss="child_data_list" id="child_data_list" v-for="(overtime,index) in overtime_list"  v-if="index > 2">
                <a  @click="staff_click(3,overtime.emp_no)" v-bind:href="staffAttend">
                    <div class="data_number">@{{ index+1 }}</div>
                    <div class="data_img">{{--<img  v-bind:src="leave_head_img">--}}
					<img v-bind:src="overtime.emp_no | empimg" :onerror="user_img_default" >
					</div>
                    <div class="data_name">@{{ overtime.emp_name }}</div>
                    <div class="data_time">@{{ overtime.hours | time }}</div>
                </a>
            </div>
...
@section('js')
    ...
    <script>
        new Vue({
            el:'title',
            data:{
                page_title:'{{trans('attendance.overtime_ranking')}}'
            }
        })
    </script>
    <script>
