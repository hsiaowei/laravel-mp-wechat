...
@section('content')
    <div id="attendance" v-cloak>
        ...
        <mt-navbar v-model="selected">
            <mt-tab-item id="1" v-model="selected"><span><i
                            class="fa fa-calendar-check-o"></i> {{trans('details.attendance_title')}}</span>
                <div class="tab_card_summary">@{{signall | time_unit_h}}</div>
            </mt-tab-item>
            <mt-tab-item id="2"><span><i class="el-icon-time"></i> {{trans('details.leave_title')}}</span>
                <div class="tab_card_summary">@{{leaveall | time_unit_h}}</div>
            </mt-tab-item>
            <mt-tab-item id="3"><span><i class="el-icon-edit"></i> {{trans('details.overtime_title')}}</span>
                <div class="tab_card_summary">@{{overall | time_unit_h}}</div>
            </mt-tab-item>
        </mt-navbar>
                    ...
                    <div v-for="(attendance,key) in attendance_list">
                        ...
                        <el-row v-for="item in attendance" :key="item.id">
                            ...
                            <el-col :span="10" class="leave-right">
                                ...
                                <p>{{trans('details.work_time')}}: <span>@{{ item.btime |substring_time}}-@{{ item.etime |substring_time}}</span>
                                </p>
                    ...
                    <div v-for="(leave,key) in leave_list">
                        ...
                        <el-row v-for="item in leave" :key="item.id">
                            ...
                            <el-col :span="16" class="leave-right">
                                ...
                                <p>{{trans('details.leave_time')}}: <span>@{{ item.btime |substring_time}}-@{{ item.etime |substring_time}}</span>
                                </p>
                    ...
                    <div v-for="(overtime,key) in overtime_list" :key="overtime.id">
                        ...
                        <el-row v-for="item in overtime" :key="item.id">
                            ...
                            <el-col :span="13" class="leave-right">
                                <p>{{trans('details.apply_time')}}: <span>@{{ item.btime |substring_time}}-@{{ item.etime |substring_time}}</span>
                                </p>
        ...
        <mt-popup v-model="popupButton">
            <div class="popupButton">
                ...
                <a href="#" @click="make_card(make_card1)">
                    <mt-button size="small" type="default">{{trans('details.make_card')}}</mt-button>
                </a>
                <a href="#" @click="ask_for_leave(make_card1)">
                    <mt-button size="small" type="primary">{{trans('details.ask_for_leave')}}</mt-button>
                </a>
            </div>
        </mt-popup>
        ...
        new Vue({
            ...
            methods: {
                ...
                attendance_message: function (choice_time) {//异常详情弹窗
                    ...
                    var that = this;
                    axios.get("/api/attendDetails", {params: {emp_no: that.emp_no, cday: now_data}}).then(function (res) {
                ...
                user_details: function (_toymd) {
                    var that = this;
                    axios.get("/api/attendance/canlendar-summary", {
                        params: {
                            emp_no: that.emp_no,
                            month_no: _toymd
                        }
                        ...
                        that.get_attend_info(result.month);
                    })
                },
                ...
                get_attend_info: function (choice_time) {
                    var that = this;
                    axios.get("/api/attendance/summary-detail", {
                        params: {
                            emp_no: that.emp_no,
